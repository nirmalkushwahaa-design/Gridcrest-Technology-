...
function CtaBtn({ btn, primary }: { btn: BtnProps; primary: boolean }) {
  const className = primary
    ? "rounded-full bg-white px-6 py-3 text-sm font-semibold text-[var(--brand-navy)] transition hover:bg-white/90"
    : "rounded-full border border-white/40 bg-transparent px-6 py-3 text-sm font-semibold text-white transition hover:bg-white/10";
  ...
  return (
    <section className="px-6 pb-20">
      ...
      >
        ...
        <h2
          data-no-reveal
          className="relative mx-auto mt-3 max-w-3xl text-3xl font-bold tracking-tight text-white md:text-5xl"
        ...
        <div className="relative mt-8 flex flex-wrap justify-center gap-3">
          <CtaBtn btn={primary} primary={true} />
          <CtaBtn btn={secondary} primary={false} />
        </div>
      </div>
    </section>
  );
